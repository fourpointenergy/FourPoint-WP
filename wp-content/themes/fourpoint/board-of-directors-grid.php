...
**/
$args = array(
  'post_type' => 'board-of-managers',
  'order' => 'menu_order',
  'post_status' => 'publish',
  'posts_per_page' => -1
);
$directors_qry = new WP_Query( $args );
?>
<?php if ( $directors_qry->have_posts() ) :
  $array = $directors_qry->posts;
  // Desc sort by order property
	usort($array,function($first,$second){
	    return $first->order < $second->order;
	});
	$directors_qry->posts = $array;
?>
  <section class="grid">
    <ul>
    <?php while ( $directors_qry->have_posts() ) : $directors_qry->the_post(); ?>
      <li>
