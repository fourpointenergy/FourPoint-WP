			...
			<?php while( have_rows('page_elements') ): the_row(); ?>
				...
				<?php if( get_row_layout() == 'accordion' && have_rows('sections') ): ?>
					...
					<?php while( have_rows('sections', $page_id) ): the_row(); $id = 'qa-'.uniqid(); ?>
						<div class="question">
							<h4><a href="#<?php echo $id ?>"><?php the_sub_field('title') ?> <?php svg('icon-plus') ?></a></h4>
						</div>
					...
					<?php while( have_rows('cards') ): the_row(); ?>
						<div class="box">
							<div class="shadow-border box-inner <?php the_sub_field('style') ?>">
								...
								<a href="<?php the_sub_field('button_link') ?>" class="text-arrow-link" target="<?php echo get_sub_field('new_window') ? '_blank' : '' ?>"><?php the_sub_field('button_text') ?></a>
								<?php endif; ?>
							</div>
						</div>
					<?php endwhile; ?>
